...
cmd_apply() {
  ...
  # The database authority of every runtime identity, re-applied after every
  # migration (WALLET-NATIVE-001). It replaced a blanket grant of DML on every
  # table to one role all five services shared, which made "only Core writes
  # financial state" a convention. Each service now has its own role; only
