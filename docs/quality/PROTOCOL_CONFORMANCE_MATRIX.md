...
Protocol baseline: BANZA v1.0 (27 accepted ADRs; ADR-045 draft).
...
| 12 | refund_source on paid events / LINK GET (ADR-045 DRAFT) | operator-extension | labeled "Banzami operator extension" in SDK (types.ts:619) and payment_links.rs:49; ADR-045 pending governance — never presented as BANZA-standard |
...
2. **ADR-045 refund_source**: adopt as protocol-normative vs. permanent
...
4. **TRANSACTION token retirement** (ADR-045 §8): governance to decide
