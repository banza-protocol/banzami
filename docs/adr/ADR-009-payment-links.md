...
A payment link is a shareable URL (`https://pay.banzami.co/{slug}`) backed by a `payment_links` record. The merchant creates a link, shares it anywhere, and the customer opens it in a browser to pay.
...
- Pay page requires a separate domain (`pay.banzami.co`) and TLS certificate.
