...
**What reaches the browser**: Only the pay URL (`https://pay.banzami.com/pay/{slug}`) and link ID. The pay URL is the public interaction surface — it is intended to be shared with the donor.
...
No card numbers, CVVs, or magnetic stripe data are involved anywhere. Banzami is a
**wallet-native** network: a payment moves value from the payer's Banzami wallet to
the merchant's wallet through the Banzami Core and ledger — there is no cardholder
data flow to secure, and Banzami is not a card processor.

Doa therefore has no card-data / PCI scope at all. Doa still handles financial event
records and donor identity data, which require appropriate data protection controls
