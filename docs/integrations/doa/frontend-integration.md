...
interface BanzaPanelProps {
  payUrl:    string;   // e.g. "https://pay.banzami.com/pay/abc123def"
...
This link is critical for donors on mobile — a donor cannot scan a QR displayed on the same screen. Tapping this opens `pay.banzami.com/pay/{slug}` either in the browser or, if the Banzami app is installed and handles the URL scheme, directly in the app.
...
The `BanzaPanel` never sees any Banzami API key, JWT, or webhook secret. The pay URL (`https://pay.banzami.com/pay/{slug}`) is public — it was created server-side and returned to the browser as the `token` field. The donor scanning or clicking this URL is the intended public interaction.
