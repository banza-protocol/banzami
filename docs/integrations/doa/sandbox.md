...
| Service | URL |
|---------|-----|
| API Gateway | `https://sandbox-api.banzami.org` |
| Business Dashboard | `https://sandbox-dashboard.banzami.org` |
| Pay Page | `https://pay.banzami.org` (same domain — sandbox links are isolated by environment) |
...
1. Visit `https://sandbox-dashboard.banzami.org` and register a sandbox merchant.
...
```bash
# Authenticate
curl -X POST https://sandbox-api.banzami.org/v1/auth/token \
  -H "Content-Type: application/json" \
  -d '{ "api_key": "bz_test_your_key" }'
# → { "token": "eyJ..." }

# Create AOA wallet
curl -X POST https://sandbox-api.banzami.org/v1/wallets \
...
```bash
curl -X POST https://sandbox-api.banzami.org/v1/sandbox/fund \
  -H "Authorization: Bearer $SANDBOX_JWT" \
  -H "Content-Type: application/json" \
  -d '{
    "wallet_id": "wlt_...",
...
1. Open `https://sandbox-dashboard.banzami.org`
...
```bash
curl -X POST https://sandbox-api.banzami.org/v1/payment-links/{link_id}/mark-used \
  -H "Authorization: Bearer $SANDBOX_JWT"
```
...
```bash
curl -X POST https://sandbox-api.banzami.org/v1/sandbox/simulate/payment \
  -H "Authorization: Bearer $SANDBOX_JWT" \
  -d '{
    "wallet_id":    "wlt_...",
    "amount_minor": 150000,
...
```bash
curl https://sandbox-api.banzami.org/v1/sandbox/instruments \
  -H "Authorization: Bearer $SANDBOX_JWT"
```
