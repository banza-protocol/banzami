...
> **⚠️ Reconciliation notice.** This set predates ADR-027/028/029. The current
> canonical contract is [docs/doa/settlement-contract.md](../../doa/settlement-contract.md)
> and [docs/doa/readiness.md](../../doa/readiness.md) — where they differ, those win.
> Receipts and proofs are generated by the **Banzami operator**, never by DOA;
> the application fee rate is **operator-priced** — the `SETTLEMENT` rule of the
> pricing profile the operator assigned ([ADR-057](../../adr/ADR-057-project-financial-readiness.md),
> which superseded the app-defined `application_fee_bps` of ADR-029); DOA names
