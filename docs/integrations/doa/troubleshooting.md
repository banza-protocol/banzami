...
console.log('payUrl:', payUrl);  // should be https://pay.banzami.com/pay/{slug}
```
...
**Symptom**: The App Banzami shows payment as successful. The Doa waiting screen never advances. No `payment_confirmed` event in the database.
...
**Symptom**: The App Banzami shows the payment was completed. Doa's polling loop never advances. The link appears as `ACTIVE` in Banzami's API.
