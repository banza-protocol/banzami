...
| **Store compromise / dump** (defence-in-depth) | The whole session record — including the upstream Bearer — is sealed at rest with **AES-256-GCM** under a dedicated `APP_WEB_SESSION_STORE_KEY` (distinct from the CSRF secret, the cookie id and any Consumer/Project credential; never in the browser, never logged). A Redis dump reveals ciphertext only; a tampered value fails authentication and the session is rejected. `REDIS_PLAINTEXT_CONSUMER_BEARERS=0`, `SESSION_STORE_CREDENTIAL_ENCRYPTION=PASS`, `SESSION_STORE_DECRYPT_FAILURE_FAILS_CLOSED=PASS`. Proven: the raw Redis value contains no `bearer`/JWT; corrupting it → 401. |
| **Store exposure / auth** | The stack Redis is network-isolated on the data plane with no host-published port (`SESSION_REDIS_PUBLIC_EXPOSURE=0`); the host supports Redis AUTH/ACL via `SESSION_REDIS_PASSWORD` where the deployment requires it. Session and rate-limit keys are separately namespaced (`bzweb:sess:` vs `bzweb:rl:`, `SESSION_REDIS_NAMESPACE_ISOLATION=PASS`); TTLs are enforced by the store (`SESSION_REDIS_TTL=PASS`); no session value or Bearer is ever logged or returned by `/healthz` (`SESSION_REDIS_SECRET_LOGGING=0`). |
