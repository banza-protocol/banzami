...
| **merchant dashboard** (dashboard-frontend) | **RETIRED** — `apps/dashboard` was an unrouted merchant surface that kept a secret API key in `localStorage` and called the Gateway from the browser, which §13 forbids; deleted 2026-09-12 (CAP-APP-002). Nothing in this repository builds the image. The deny entry survives only because the legacy `/srv/banzami/docker-compose.yml` on the host still declares the service. | `./deploy.sh dashboard-frontend` — **fails closed** |
