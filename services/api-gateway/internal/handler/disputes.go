...
func (h *DisputeHandler) Get(w http.ResponseWriter, r *http.Request) {
	...
	dispute, err := h.svc.Get(r.Context(), id)
	if err != nil {
...
// GET /v1/disputes?merchant_id=&consumer_id=&status=&limit=
...
func (h *DisputeHandler) SubmitEvidence(w http.ResponseWriter, r *http.Request) {
	...
	case body.SubmittedBy == "":
		...
		return
	case body.Party == "":
		apierror.Respond(w, r, http.StatusBadRequest, "MISSING_FIELD", "party is required (CONSUMER or MERCHANT)")
		return
