...
({BanzamiClient client, List<http.Request> requests}) _client(int status, Object body) {
  ...
  final client = BanzamiClient(
    ...
    httpClient: MockClient((req) async {
      ...
      if (req.url.path.endsWith('/auth/token')) {
        return http.Response(_jwtBody, 200, headers: {'content-type': 'application/json'});
      }
      reqs.add(req);
...
Map<String, dynamic> _statusJson() => {
      ...
      'verified': false,
      'documents': [
        {'document_type': 'COMMERCIAL_REGISTRATION', 'status': 'PENDING_REVIEW', 'submitted_at': '2026-06-29T10:00:00Z'},
...
void main() {
  group('getMerchantKybStatus', () {
    test('parses status + 3 typed document slots', () async {
      ...
      expect(st.documents[0].type, MerchantKybDocumentType.commercialRegistration);
  ...
  group('requestMerchantKybDocumentUploadUrl', () {
    test('hits /documents/{type}/upload-url, sends content_type, parses + redacts URL', () async {
      final h = _client(200, {
        'document_id': 'doc-1',
        'upload_url': 'https://r2.example/kyb/merchant/abc?X-Amz-Signature=secret',
        'method': 'PUT',
      ...
      expect(req.url.path, '/v1/merchant/kyb/documents/COMPANY_TAX_ID/upload-url');
      expect((jsonDecode(req.body) as Map)['content_type'], 'image/jpeg');
    });

    test('503 STORAGE_NOT_CONFIGURED', () async {
      final h = _client(503, {'code': 'STORAGE_NOT_CONFIGURED', 'message': 'no storage'});
      expect(
        () => h.client.requestMerchantKybDocumentUploadUrl(MerchantKybDocumentType.companyTaxId),
        throwsA(isA<BanzamiApiException>().having((e) => e.statusCode, 'status', 503)),
      );
    });
  });
  ...
  group('completeMerchantKybDocumentUpload', () {
    test('returns the updated document (PENDING_REVIEW)', () async {
      final h = _client(200, {'id': 'doc-1', 'document_type': 'COMPANY_TAX_ID', 'status': 'PENDING_REVIEW'});
      final d = await h.client.completeMerchantKybDocumentUpload('doc-1');
      expect(d.status, MerchantKybDocumentStatus.pendingReview);
      expect(h.requests.single.url.path, '/v1/merchant/kyb/documents/doc-1/complete');
    });

    test('404 for another merchant\'s document', () async {
      final h = _client(404, {'code': 'NOT_FOUND', 'message': 'document not found'});
      expect(
  ...
  group('enums', () {
    test('wire round-trips', () {
      expect(MerchantKybDocumentType.fromWire('REPRESENTATIVE_ID'), MerchantKybDocumentType.representativeId);
      expect(MerchantKybDocumentType.fromWire('NOPE'), isNull);
      expect(MerchantKybDocumentStatus.fromWire('EXPIRED'), MerchantKybDocumentStatus.expired);
      expect(MerchantKybDocumentStatus.fromWire('WAT'), MerchantKybDocumentStatus.unknown);
    });
  });
}
