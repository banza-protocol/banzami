...
class _CheckoutScreenState extends State<CheckoutScreen> {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      backgroundColor: BanzamiTheme.offWhite,
      appBar: AppBar(
        backgroundColor: Colors.transparent,
        elevation: 0,
        leading: IconButton(
          icon: const Icon(Icons.close, color: BanzamiTheme.gray700),
          onPressed: () {
        ...
        title: Text(
          'Banzami Pay',
          style: TextStyle(color: BanzamiTheme.gray900, fontWeight: FontWeight.w600),
        ),
        centerTitle: true,
      ),
      body: _buildBody(),
  ...
  Widget _buildBody() {
    if (_loading) {
      return const Center(child: CircularProgressIndicator(color: BanzamiTheme.wine));
    }
    if (_error != null) {
      return Center(
        child: Padding(
        ...
        Container(
          ...
          child: Column(children: [
            const Text('Digitalize o código QR com a app Banzami',
                style: TextStyle(fontSize: 13, color: BanzamiTheme.gray700),
                textAlign: TextAlign.center),
            const SizedBox(height: 16),
            QrImageView(
              data:            deepLink,
            ...
            SizedBox(
              ...
              child: ElevatedButton(
                ...
                style: ElevatedButton.styleFrom(
                  backgroundColor: BanzamiTheme.wine,
            ...
            const Row(mainAxisAlignment: MainAxisAlignment.center, children: [
              SizedBox(width: 14, height: 14,
                  child: CircularProgressIndicator(strokeWidth: 2,
                      color: BanzamiTheme.gray400)),
              SizedBox(width: 8),
              Text('A aguardar confirmação…',
                  style: TextStyle(fontSize: 12, color: BanzamiTheme.gray400)),
            ]),
          ]),
        ),
      ]),
      ...
      child: Column(mainAxisSize: MainAxisSize.min, children: [
        ...
        const Text('Pagamento confirmado!',
            style: TextStyle(fontSize: 20, fontWeight: FontWeight.bold,
                color: BanzamiTheme.gray900)),
        const SizedBox(height: 8),
        const Text('Obrigado pelo pagamento.',
            style: TextStyle(color: BanzamiTheme.gray400)),
      ]),
    );
  }
}
      ...
      child: Padding(
        ...
        child: Column(mainAxisSize: MainAxisSize.min, children: [
          ...
          const Text('Link inválido',
              style: TextStyle(fontSize: 20, fontWeight: FontWeight.bold)),
          const SizedBox(height: 8),
          Text(msg, style: const TextStyle(color: BanzamiTheme.gray400),
              textAlign: TextAlign.center),
        ]),
      ),
    );
