...
// Shared final call-to-action block (the design's BanzamiCTAFooter, minus the footer).
export function CTASection({ id }: { id?: string }) {
  return (
    <section id={id} className="px-6 py-[clamp(56px,8vw,100px)]">
      <Reveal className="relative mx-auto max-w-[1080px] overflow-hidden rounded-[36px] bg-gradient-to-br from-banzami to-banzami-deep p-[clamp(40px,6vw,76px)] text-center">
        ...
        <div className="relative">
          <h2 className="m-0 text-[clamp(32px,5vw,56px)] font-black leading-[1.02] tracking-[-0.025em] text-white">
            Constrói connosco.
          </h2>
          ...
          <div className="mt-[30px] flex flex-wrap justify-center gap-3">
            <a
              href={mailto()}
              className="inline-flex items-center gap-2 rounded-pill bg-white px-[30px] py-4 text-[16px] font-extrabold text-banzami no-underline transition-transform hover:-translate-y-0.5"
            >
              Falar connosco
            </a>
            <a
              href={mailto('Waitlist Banzami')}
              className="inline-flex items-center gap-2 rounded-pill border border-white/30 bg-white/[0.14] px-7 py-4 text-[16px] font-extrabold text-white no-underline transition-colors hover:bg-white/20"
            >
              Entrar na waitlist
            </a>
          </div>
