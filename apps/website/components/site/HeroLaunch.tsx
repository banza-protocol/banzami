...
export function HeroLaunch({ sandboxName, liveSummaryShort }: { sandboxName: string; liveSummaryShort: string }) {
  ...
  return (
    <section id="inicio" className="relative overflow-hidden bg-white px-6 pb-10 pt-[96px] md:pt-[104px]">
      ...
      <div className="relative mx-auto grid w-full max-w-container grid-cols-1 items-center gap-y-14 lg:grid-cols-[minmax(0,600px)_minmax(0,1fr)] lg:gap-x-[clamp(40px,4vw,72px)]">
        ...
        <div className="max-w-[600px]">
          ...
          <h1 className="m-0 mt-[clamp(22px,2.6vh,30px)] text-[clamp(38px,3.4vw+1vh,62px)] font-black leading-[1.02] tracking-[-0.03em] text-ink">
            O novo caminho do <span className="text-cherry">Kwanza.</span>
          </h1>

          <p className="m-0 mt-[clamp(20px,2.4vh,28px)] max-w-[540px] text-[clamp(15px,0.5vw+0.7vh,18px)] font-semibold leading-[1.6] text-ink-secondary">
            Envie, receba e aceite pagamentos em Kz entre pessoas, negócios e aplicações — na app
            Banzami ou integrado no seu produto.
          </p>

          <p data-testid="home-environment-status" className="m-0 mt-[clamp(14px,1.8vh,20px)] max-w-[520px] text-[12.5px] font-semibold text-ink-muted">
            {sandboxName} disponível com dinheiro fictício. {liveSummaryShort}
          </p>

          {/* CTAs */}
          <div className="mt-[clamp(22px,2.8vh,30px)] flex flex-wrap items-center gap-3">
            <a
              href={APP_WEB}
              target="_blank"
          ...
          {/* Audience cards */}
          <div className="mt-[clamp(24px,3vh,32px)] grid max-w-[560px] grid-cols-1 gap-3 sm:grid-cols-2">
            <AudienceCard
          ...
          <div className="mt-[clamp(28px,3.4vh,38px)] max-w-[560px]">
            ...
            <p className="m-0 mt-[clamp(18px,2.2vh,26px)] flex items-center gap-2 text-[12.5px] font-semibold text-ink-muted">
