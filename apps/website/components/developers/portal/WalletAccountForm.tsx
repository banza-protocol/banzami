...
// Core's list, minus PRIMARY, which is made with the wallet and cannot be
// opened here. Each carries the word a developer would actually use for it.
const PURPOSES: Array<{ v: string; label: string; hint: string }> = [
  { v: 'CAMPAIGN', label: 'Campanha', hint: 'uma angariação, uma causa, um projeto com meta' },
  { v: 'STORE', label: 'Loja', hint: 'um vendedor ou uma loja dentro da sua aplicação' },
  { v: 'PROJECT', label: 'Projeto', hint: 'um trabalho ou encomenda com fundos próprios' },
  { v: 'EVENT', label: 'Evento', hint: 'bilheteira ou inscrições de um evento' },
  { v: 'ESCROW', label: 'Caução', hint: 'valor retido até uma condição se cumprir' },
  { v: 'RESERVE', label: 'Reserva', hint: 'valor separado que não deve ser gasto' },
  { v: 'SETTLEMENT', label: 'Liquidação', hint: 'valor a caminho de um beneficiário' },
  { v: 'CUSTOM', label: 'Outro', hint: 'quando nenhuma das anteriores descreve o seu caso' },
];
...
export function WalletAccountForm({ onCreated }: { onCreated: () => void }) {
  ...
  const chosen = PURPOSES.find((p) => p.v === purpose);
