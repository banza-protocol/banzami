...
/**
 * Hero phone (handoff "Banza App Demo" with open-url): shows the intro screen
 * and opens the REAL web app (app.banzami.com) on click. No inline demo — the
 * app web is real. Decorative frame; the whole screen is a link.
 */
export function HeroAppPhone({ lang }: { lang: Lang }) {
  ...
  return (
    <div style={{ position: 'relative', filter: 'drop-shadow(0 50px 60px rgba(60,0,8,.45))' }}>
      <div style={{ position: 'relative', width: '300px', height: '620px', borderRadius: '48px', background: '#160a0c', padding: '9px', boxShadow: '0 40px 80px -30px rgba(122,16,22,.45),0 0 0 1px rgba(122,16,22,.08)', animation: 'floaty 6.5s ease-in-out infinite' }}>
        ...
        <div style={{ position: 'relative', width: '100%', height: '100%', borderRadius: '40px', overflow: 'hidden' }}>
          <a
            ...
            aria-label={t.aria}
            style={{ position: 'absolute', inset: 0, cursor: 'pointer', display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', textAlign: 'center', padding: '34px', background: 'radial-gradient(circle at 50% 28%,#C81824,#9A1B22 58%,#6E0E14)', textDecoration: 'none' }}
          >
