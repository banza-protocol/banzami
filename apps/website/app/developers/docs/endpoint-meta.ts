...
export const ENDPOINT_META: Record<string, EndpointMeta> = {
  ...
  'ref-realtime-status': {
    ...
    params: [
      { in: 'header', name: 'Authorization', type: 'string', required: true,
        note: { pt: 'Bearer e o realtime.token da sessão (bzst_…). Nunca uma chave, nunca no endereço', en: 'Bearer and the session’s realtime.token (bzst_…). Never a key, never in the URL' },
        example: 'Bearer bzst_XXXXXXXXXXXXXXXX' },
      { in: 'header', name: 'Accept', type: 'string', required: false,
        note: { pt: 'text/event-stream para o stream; application/json (por omissão) para uma leitura única', en: 'text/event-stream for the stream; application/json (default) for a single read' },
        example: 'text/event-stream' },
      pathId({ pt: 'o id da sessão de pagamento', en: 'the Payment Session id' }, 'payment_session_exemplo'),
    ],
    sdk: null,
    sdkNote: {
      pt: 'No browser: watchPaymentSessionStatus de @banzami/sdk/realtime, com o token — não leva chave e recusa uma. No servidor, leia a sessão com getPaymentSession.',
      en: 'In the browser: watchPaymentSessionStatus from @banzami/sdk/realtime, with the token — it takes no key and refuses one. On the server, read the session with getPaymentSession.',
    },
    events: [],
    guides: ['payments', 'testing'],
  },
  'ref-sandbox-scenarios': {
    scope: 'sandbox:read',
    params: [AUTH],
    sdk: 'listSandboxScenarios',
    events: [],
    guides: ['testing'],
  },
  'ref-test-payer-create': {
    ...
    params: [
      AUTH, IDEM_HEADER,
      { in: 'body', name: 'label', type: 'string', required: false,
        note: { pt: 'um nome para o reconhecer, até 60 caracteres', en: 'a name to recognise it by, up to 60 characters' }, example: 'Maria (teste)' },
      { in: 'body', name: 'initial_balance_minor', type: 'integer', required: false,
        note: { pt: 'saldo fictício inicial, de 0 a 1 000 000; 1 000 000 por omissão', en: 'fictitious opening balance, 0 to 1,000,000; 1,000,000 by default' }, example: '1000000' },
    ],
    sdk: 'createTestPayer',
    events: [],
    guides: ['testing', 'get-started'],
  },
  'ref-test-payer-list': {
    scope: 'sandbox:read',
    params: [
      AUTH,
      { in: 'query', name: 'include_retired', type: 'boolean', required: false,
        note: { pt: 'true para incluir os pagadores retirados', en: 'true to include retired payers' }, example: 'true' },
    ],
    sdk: 'listTestPayers',
    events: [],
    guides: ['testing'],
  ...
  'ref-test-payer-get': {
    scope: 'sandbox:read',
    params: [AUTH, pathId({ pt: 'o id do pagador de teste', en: 'the test payer id' }, 'tp_exemplo')],
    sdk: 'getTestPayer',
    events: [],
    guides: ['testing'],
  },
  'ref-test-payer-fund': {
    ...
    params: [
      AUTH,
      { ...IDEM_HEADER, required: true,
        note: { pt: 'obrigatório: identifica o carregamento; a mesma chave nunca carrega duas vezes', en: 'required: identifies the top-up; the same key never funds twice' },
        example: 'carregamento_001' },
      pathId({ pt: 'o id do pagador de teste', en: 'the test payer id' }, 'tp_exemplo'),
      { in: 'body', name: 'amount_minor', type: 'integer', required: true,
        note: { pt: 'de 1 a 2 500 000 unidades menores', en: '1 to 2,500,000 minor units' }, example: '500000' },
    ],
    sdk: 'fundTestPayer',
    events: [],
    guides: ['testing'],
  ...
  'ref-test-payer-pay': {
    ...
    params: [
      AUTH,
      { ...IDEM_HEADER,
        note: { pt: 'obrigatório com simulate TIMEOUT; recomendado sempre — repetir com a mesma chave lê o resultado', en: 'required with simulate TIMEOUT; recommended always — repeating with the same key reads the result' },
        example: 'pagamento_001' },
      pathId({ pt: 'o id do pagador de teste', en: 'the test payer id' }, 'tp_exemplo'),
      { in: 'body', name: 'payment_session_id', type: 'string', required: 'conditional',
        note: { pt: 'a sessão a pagar, do seu projeto; ou payment_link_id', en: 'the session to pay, your Project’s own; or payment_link_id' }, example: 'payment_session_exemplo' },
      { in: 'body', name: 'via', type: 'string (enum)', required: false,
        note: { pt: 'para uma sessão: LINK (por omissão) ou QR', en: 'for a session: LINK (default) or QR' }, example: 'QR' },
      { in: 'body', name: 'payment_link_id', type: 'string', required: 'conditional',
        note: { pt: 'o link a pagar, do seu projeto; ou payment_session_id', en: 'the link to pay, your Project’s own; or payment_session_id' }, example: 'plink_exemplo' },
      { in: 'body', name: 'amount_minor', type: 'integer', required: 'conditional',
        note: { pt: 'só para um montante aberto', en: 'only for an open amount' }, example: '250000' },
      { in: 'body', name: 'simulate', type: 'string (enum)', required: false,
        note: { pt: 'DECLINED, PROVIDER_UNAVAILABLE ou TIMEOUT: um resultado de rede externa, pedido explicitamente', en: 'DECLINED, PROVIDER_UNAVAILABLE or TIMEOUT: an external-rail outcome, requested explicitly' }, example: 'TIMEOUT' },
    ],
    sdk: 'payAsTestPayer',
    events: ['payment_session.paid', 'payment_link.paid'],
    guides: ['testing', 'payments'],
  },
  'ref-test-payer-retire': {
    scope: 'sandbox:write',
    params: [AUTH, pathId({ pt: 'o id do pagador de teste', en: 'the test payer id' }, 'tp_exemplo')],
    sdk: 'retireTestPayer',
    events: [],
