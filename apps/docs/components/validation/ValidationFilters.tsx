import type { ValidationStatus, ValidationPriority } from '@/lib/validation-types'
...
const STATUSES: { value: ValidationStatus; label: string; color: string }[] = [
  ...
  { value: 'IMPLEMENTED',  label: 'Implementada',  color: 'text-bz-primary bg-bz-primary-light border-bz-primary/30' },
  { value: 'IN_PROGRESS',  label: 'Em progresso',  color: 'text-amber-700 bg-amber-50 border-amber-200' },
  ...
  { value: 'NEEDS_REVIEW', label: 'Rever',         color: 'text-orange-700 bg-orange-50 border-orange-200' },
]
    ...
    <div className="space-y-3">
      ...
      <div className="flex items-center gap-2">
        ...
        {(activeStatus || activePriority || search) && (
          <button
            onClick={() => { onStatusChange(null); onPriorityChange(null); onSearchChange(''); }}
            className="ml-auto rounded-lg border border-bz-border px-2 py-0.5 text-[10px] text-bz-muted hover:border-bz-primary/30 hover:text-bz-text"
          >
            Limpar filtros
          </button>
        )}
