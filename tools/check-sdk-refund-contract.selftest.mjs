...
// The retired mount, assembled rather than written whole. A repository-wide path
// sweep once rewrote every '/business/refunds' in this file into '/refunds', which
// turned each mutation below into a no-op: the "bad" client was the good client,
// the guard passed it, and the self-test reported the guard as broken when it was
// the test that had been disarmed. A sweep cannot rewrite what is not spelled out.
const RETIRED = '/busi' + 'ness/refunds';
...
{ const bad = GOOD_CLIENT.replace("this.request('/refunds'", `this.request('${RETIRED}'`);
  const r = runGuard(bad, GOOD_TYPES, '0.3.0');
...
//    is what breaks.
{ const bad = GOOD_CLIENT.replace('/refunds/', `${RETIRED}/`);
  const r = runGuard(bad, GOOD_TYPES, '0.3.0');
  check('getRefund on the retired path → fail', !r.pass && /business\/refunds/.test(r.out)); }

// 9. The list side alone.
{ const bad = GOOD_CLIENT.replace("'/refunds' + this.qs", `'${RETIRED}' + this.qs`);
  const r = runGuard(bad, GOOD_TYPES, '0.3.0');
...
//     the path moved.
{ const ok = `\n    // Was ${RETIRED} until the route moved.\n` + GOOD_CLIENT;
...
{ const bad = GOOD_CLIENT.replace("this.request('/refunds',", `this.request('${RETIRED}', // legacy\n           `);
  const r = runGuard(bad, GOOD_TYPES, '0.3.0');
