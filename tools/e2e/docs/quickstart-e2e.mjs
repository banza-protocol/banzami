...
async function complete() {
  ...
  // workspace revokes every key, retires the test payers and the test Business
  // through Core and disables the webhook; nothing is archived.
  const ws = await call(`/workspaces/${state.created.workspace}`);
  cleanup.push(`workspace deleted: ${(await call(`/workspaces/${state.created.workspace}`, 'DELETE', { name: ws.body?.name })).status}`);
