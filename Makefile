...
# A negative assertion whose subject no longer exists proves nothing and says
# PASS. Every *_MARKER an app-web proof declares must still be rendered by the
# Flutter source — a positive gate turns red and gets investigated, a negative
# one goes quietly green forever.
...
.PHONY: check-validation-assurance-adapter
...
# A balance that could not be read is not a balance of zero. proof 06 coerced a
# null reading with `?? 0` and then waited 25 s for a consumer holding 10 000 Kz
# to display 100 — reporting "B did not update" about a product that was fine.
.PHONY: check-e2e-measurement-honesty
...
.PHONY: check-validation-concurrent-peak
# The peak is what was held AT ONCE, by a COMPLETE ownership set, in an order
# the ledger chose. S23 declared a project and a consumer, resolved perfectly,
# and omitted the test payer holding 1 200 000.
check-validation-concurrent-peak:
	node tools/check-validation-concurrent-peak.selftest.mjs

.PHONY: check-validation-workspace-capacity
# Workspace capacity is TWO resources, per actor, and neither was gated when
# BZV-20260921-0001 was abandoned against the one that refuses. ACTIVE is
# concurrency and archiving frees it; 24H CREATION is cumulative over a sliding
# window and archiving frees NONE of it. One formula for both is the defect.
...
.PHONY: check-validation-limiter-reading
# An empty application-submit window is a READING, not an absence of one. Read
# as unknown, it closed the owner gate at the moment the resource was maximally
# available: thirty free slots reported as "no record of this machine".
check-validation-limiter-reading:
...
# owning a Business arrived with a valid manifest and an UNKNOWN verdict.
check-validation-resource-scope:
	node tools/check-validation-resource-scope.selftest.mjs

# Ownership reachable + every ownable kind classified + every FINANCIAL kind
# resolvable. Static, because a matrix that needs a FULL cannot gate one.
check-validation-ownership-matrix:
	node tools/validation-ownership-matrix.mjs
...
# declare its consumer leaked 296 000 minor while reporting FUNCTIONAL PASS.
check-e2e-ownership-adoption:
	node tools/check-e2e-ownership-adoption.selftest.mjs

.PHONY: check-validation-journey-counters
# A CONTROL row must not move a JOURNEY counter. BZV-20260920-0001 printed
# "17 passed / 2 failed / 20 not reached" for 38 journeys: the outcome counters
# were unscoped while the structural ones in the same SELECT were not.
check-validation-journey-counters:
