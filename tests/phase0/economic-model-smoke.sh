...
#
# Settlement cases B, C, E and G used to POST source_wallet_id /
# beneficiary_wallet_id / application_fee_wallet_id. /v1/application-settlements
# takes none of those: it takes a segregated ACCOUNT the caller owns and two
# @banza names. Every settlement here answered 400 MISSING_FIELD, and every
# economic assertion after it measured a settlement that never happened.
#
# So each settling owner now has what the contract names:
#   source        a CAMPAIGN account it owns — funded by an admin credit to its
#                 PRIMARY account moved across with a wallet-account transfer,
#                 because a settlement refuses a PRIMARY source
#   beneficiary   another Business's @banza
#   fee dest      the owner's OWN @banza (ADR-028 refuses anyone else's)
# and the funds it was given are retired when the run ends (tests/phase0/lib/
# e2e-run.sh), so a run leaves the pilot funding cap where it found it.
set -uo pipefail
...
mk(){ # $1 = profile code or "none" -> prints merchant|wallet|campaign_account|@banza
  local mid wid
  ...
  wid=$(jget id)
  # Nameable: a beneficiary and a fee destination are both named by @banza.
  local h
  h=$(printf 'm%s' "$(printf '%s' "$mid" | tr -d '-' | cut -c1-11)" | tr 'A-Z' 'a-z')
  ...
  # The segregated account a settlement moves value out of.
  call "$GW" 8080 POST /v1/wallet-accounts \
    "{\"wallet_id\":\"$wid\",\"purpose\":\"CAMPAIGN\",\"reference_type\":\"ECONOMIC_SMOKE\",\"reference_id\":\"src-$mid\",\"label\":\"economic smoke source\"}" "$(mint "$mid")"
...
# Value into the CAMPAIGN account the way an owner puts it there: onto the
# PRIMARY account, then across with a wallet-account transfer.
fund_campaign(){ # $1=merchant $2=wallet $3=campaign_account $4=amount
  local primary
  fund "$2" "$4"
  ...
  call "$GW" 8080 POST /v1/wallet-account-transfers \
    "{\"source_wallet_account_id\":\"$primary\",\"destination_wallet_account_id\":\"$3\",\"amount_minor\":$4,\"currency\":\"AOA\",\"idempotency_key\":\"fund-$3\",\"description\":\"economic smoke $R\"}" "$(mint "$1")"
}

settle(){ # $1=merchant $2=source_account $3=beneficiary_@banza $4=idem [$5=fee_@banza]
  ...
  [ -n "${5:-}" ] && fee=",\"fee_destination_banza_name\":\"$5\""
  call "$GW" 8080 POST /v1/application-settlements \
    "{\"idempotency_key\":\"set-$4-$R\",\"source_account_id\":\"$2\",\"beneficiary_banza_name\":\"$3\",\"reason\":\"ECONOMIC_SMOKE\",\"reference_type\":\"ECONOMIC_SMOKE\",\"reference_id\":\"ref-$4-$R\"$fee}" "$jwt"
}

echo
echo "### A — a payment credits the wallet GROSS, with a matching rule present"
IFS='|' read -r AM AW _ _ <<<"$(mk sandbox-reference)"
...
IFS='|' read -r BM BW BA _ <<<"$(mk sandbox-default)"
IFS='|' read -r _ BBW _ BBH <<<"$(mk sandbox-default)"
fund_campaign "$BM" "$BW" "$BA" "$GROSS"
chk B_SOURCE_FUNDED "$(abal "$BA")" "$GROSS"
settle "$BM" "$BA" "$BBH" b
...
chk B_BENEFICIARY_GETS_GROSS "$(wbal "$BBW")" "$GROSS"
chk B_SOURCE_EMPTIED "$(abal "$BA")" "0"

echo
echo "### C — sandbox-reference settlement: 200 bps, fee 2000, net 98000"
EXPECTED_FEE=$(( GROSS * 200 / 10000 ))
IFS='|' read -r CM CW CA CH <<<"$(mk sandbox-reference)"
IFS='|' read -r _ CBW _ CBH <<<"$(mk sandbox-reference)"
fund_campaign "$CM" "$CW" "$CA" "$GROSS"
chk C_SOURCE_FUNDED "$(abal "$CA")" "$GROSS"
# Gross leaves the campaign account, the beneficiary receives the net, and the
# fee lands in the owner's own PRIMARY account, named by its own @banza.
settle "$CM" "$CA" "$CBH" c "$CH"
...
chk C_BENEFICIARY_GETS_NET "$(wbal "$CBW")" "$(( GROSS - EXPECTED_FEE ))"
chk C_FEE_TO_OWN_ACCOUNT "$(wbal "$CW")" "$EXPECTED_FEE"
chk C_SOURCE_EMPTIED "$(abal "$CA")" "0"
# One posting per leg, and nothing half-written.
chk C_POSTINGS_RECORDED "$(q "SELECT (settlement_posting_id IS NOT NULL AND fee_posting_id IS NOT NULL)::text FROM app_settlements WHERE id='$CSID'")" "true"
# The same request again is the same settlement, not a second one.
settle "$CM" "$CA" "$CBH" c "$CH"
chk C_REPLAY_IS_THE_SAME_SETTLEMENT "$(jget id)" "$CSID"
chk C_REPLAY_MOVES_NOTHING "$(wbal "$CBW")" "$(( GROSS - EXPECTED_FEE ))"
chk C_ONE_SETTLEMENT_FOR_THE_KEY "$(q "SELECT count(*) FROM app_settlements WHERE idempotency_key LIKE '%set-c-$R'")" "1"
...
IFS='|' read -r DM DW _ _ <<<"$(mk sandbox-default)"
fund "$DW" "$GROSS"
...
IFS='|' read -r EM EW EA _ <<<"$(mk none)"
IFS='|' read -r _ EBW _ EBH <<<"$(mk sandbox-default)"
fund_campaign "$EM" "$EW" "$EA" "$GROSS"
...
chk E_SOURCE_UNTOUCHED "$(abal "$EA")" "$GROSS"
chk E_BENEFICIARY_UNTOUCHED "$(wbal "$EBW")" "0"
...
IFS='|' read -r FM FW _ _ <<<"$(mk "smoke-settle-only-$R")"
fund "$FW" "$GROSS"
...
IFS='|' read -r GM GW2 GA GH <<<"$(mk sandbox-reference)"
IFS='|' read -r _ GBW _ GBH <<<"$(mk sandbox-reference)"
fund_campaign "$GM" "$GW2" "$GA" "$GROSS"
q "INSERT INTO pricing_rules (id, rule_key, version, environment, enabled, pricing_profile,
                              pricing_operation, rate_bps, flat_minor, rounding, priority,
...
settle "$GM" "$GA" "$GBH" g "$GH"
chk G_SETTLEMENT_REFUSED "$CODE" "409"
chk G_REFUSAL_IS_NAMED   "$(errcode)" "PRICING_CONFIGURATION_ERROR"
chk G_SOURCE_UNTOUCHED   "$(abal "$GA")" "$GROSS"
chk G_BENEFICIARY_UNTOUCHED "$(wbal "$GBW")" "0"
