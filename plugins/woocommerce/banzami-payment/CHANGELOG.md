...
### Added
- WooCommerce payment gateway class integrating Banzami's `POST /v1/transactions` API
...
- Order meta storage: `_banzami_transaction_id` (Banzami transaction UUID), `_banzami_amount_minor` (integer minor units sent to API), `_banzami_refund_processed` (refund idempotency flag)
