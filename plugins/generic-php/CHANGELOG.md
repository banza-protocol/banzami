...
### Added
- `BanzamiClient` class supporting PHP 7.4+ with no external dependencies (only `ext-curl` and `ext-json` required)
...
- Static webhook helper: `BanzamiClient::verifyWebhookSignature` — HMAC-SHA256 verification using `hash_equals` for timing-safe comparison
- `WebhookHandler` class: `parse(string $rawBody, string $signature): array` — verifies signature and decodes the event in one call
- Static money helpers: `BanzamiClient::formatAmount(int $amountMinor, string $currency): string` and `BanzamiClient::toMinorUnits(float $total, string $currency): int`
