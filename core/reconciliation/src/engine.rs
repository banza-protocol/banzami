...
use banzami_types::{Currency, ReconciliationRunId, SettlementId};

use crate::{
    repository::ReconciliationRepository, ExternalStatementLine, ReconciliationError,
    ReconciliationRecord, ReconciliationReport, ReconciliationStatus,
};

// ---------------------------------------------------------------------------
// Minimal settlement view — avoids a hard dependency on banzami-settlement
// ---------------------------------------------------------------------------
...
impl<R: ReconciliationRepository> ReconciliationEngine for StaticReconciliationEngine<R> {
    async fn run(
        ...
        for settlement in &internal_settlements {
            ...
            let matched_idx = external_lines.iter().enumerate().find(|(i, ext)| {
                !consumed[*i]
            ...
            if let Some((idx, ext)) = matched_idx {
                ...
                let mismatch_idx = external_lines
                    .iter()
                    .enumerate()
                    .find(|(i, ext)| !consumed[*i] && ext.currency == currency_code);
        ...
        let matched = records
            ...
            .count() as u64;
        let missing_external = records
            .iter()
            .filter(|r| r.status == ReconciliationStatus::MissingExternal)
            .count() as u64;
        let missing_internal = records
            .iter()
            .filter(|r| r.status == ReconciliationStatus::MissingInternal)
            .count() as u64;
        let amount_mismatches = records
            .iter()
            .filter(|r| r.status == ReconciliationStatus::AmountMismatch)
            .count() as u64;
...
mod tests {
    ...
    async fn perfect_match_is_reported_as_matched() {
        let report = engine()
            .run(vec![ext("REF-001", 100_000)], vec![int(100_000)])
            .await
            .unwrap();
    ...
    async fn report_is_persisted_and_retrievable() {
        let eng = engine();
        let report = eng
            .run(vec![ext("R1", 10_000)], vec![int(10_000)])
            .await
            .unwrap();
