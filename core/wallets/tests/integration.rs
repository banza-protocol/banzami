...
use banzami_wallets::{
    ...
    ReleaseRequest, ReserveRequest, SettleRequest, WalletEngine,
};
...
async fn fresh_wallet_has_zero_balance(pool: PgPool) -> sqlx::Result<()> {
    ...
    let wallet = engine.create(CreateWalletRequest {
        merchant_id,
...
async fn reserve_increases_reserved_balance(pool: PgPool) -> sqlx::Result<()> {
    ...
    let transit = ledger.create_account(
        Account::new(AccountType::Asset, "transit-float", Currency::AOA)
    ).await.unwrap();

    let wallet = engine.create(CreateWalletRequest { merchant_id, currency: Currency::AOA }).await.unwrap();

    engine.reserve(ReserveRequest {
    ...
    let balance = engine.balance(wallet.id).await.unwrap();
    assert_eq!(balance.available.amount_minor(), 0,        "available must not change on reserve");
    assert_eq!(balance.reserved.amount_minor(),  50_000_00,"reserved must increase");
    assert_eq!(balance.total.amount_minor(),     50_000_00,"total = available + reserved");
...
async fn reserve_is_idempotent(pool: PgPool) -> sqlx::Result<()> {
    ...
    let transit = ledger.create_account(
        Account::new(AccountType::Asset, "transit-idem", Currency::AOA)
    ).await.unwrap();
    ...
    let balance = engine.balance(wallet.id).await.unwrap();
    assert_eq!(balance.reserved.amount_minor(), 10_000_00, "idempotent reserve must not double-credit");
...
async fn settle_moves_reserved_to_available(pool: PgPool) -> sqlx::Result<()> {
    ...
    let transit = ledger.create_account(
        Account::new(AccountType::Asset, "transit-settle", Currency::AOA)
    ).await.unwrap();
    ...
    let balance = engine.balance(wallet.id).await.unwrap();
    assert_eq!(balance.available.amount_minor(), 100_000_00, "available must increase after settle");
    assert_eq!(balance.reserved.amount_minor(),  0,          "reserved must clear after settle");
...
async fn release_restores_available_after_reserve(pool: PgPool) -> sqlx::Result<()> {
    ...
    let transit = ledger.create_account(
        Account::new(AccountType::Asset, "transit-release", Currency::AOA)
    ).await.unwrap();
    ...
    let balance = engine.balance(wallet.id).await.unwrap();
    assert_eq!(balance.available.amount_minor(), 0, "available must be zero after release");
    assert_eq!(balance.reserved.amount_minor(),  0, "reserved must clear after release");
    assert_eq!(balance.total.amount_minor(),     0, "total must be zero — funds returned to transit");
