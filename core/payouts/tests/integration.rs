...
use banzami_payouts::{
    BankDestination, CreatePayoutRequest, PayoutEngine, PayoutStatus,
    PostgresPayoutEngine, PostgresPayoutRepository,
};
use banzami_types::{AccountId, Currency, MerchantId, Money, WalletId};
use banzami_wallets::{
    CreateWalletRequest, PostgresWalletEngine, PostgresWalletRepository,
...
struct TestFixture {
    payout_engine: PostgresPayoutEngine<PostgresWalletRepository, PostgresLedgerRepository, PostgresPayoutRepository>,
...
async fn setup(pool: PgPool) -> TestFixture {
    ...
    let bank = ledger.create_account(
        Account::new(AccountType::Asset, "bank-payout-test", Currency::AOA)
    ).await.unwrap();

    let transit = ledger.create_account(
        Account::new(AccountType::Asset, "transit-payout-test", Currency::AOA)
    ).await.unwrap();
    ...
    wallet_engine.reserve(ReserveRequest {
        idempotency_key: "setup-reserve".into(),
        wallet_id:       wallet.id,
        amount:          kz(1_000_000_00),
        from_account_id: transit.id,
    }).await.unwrap();
    wallet_engine.settle(SettleRequest {
    ...
    TestFixture { payout_engine, ledger, merchant_id, wallet_id: wallet.id, bank_id: bank.id }
}
...
async fn initiate_to_confirmed_happy_path(pool: PgPool) -> sqlx::Result<()> {
    ...
    assert_eq!(bank_balance_before.amount_minor(), 0, "no ledger entry before process");

    let processing = fix.payout_engine.process(payout.id).await.unwrap();
    assert_eq!(processing.status, PayoutStatus::Processing);
    assert!(processing.ledger_posting_id.is_some(), "processing must record a posting ID");
    ...
    assert_eq!(
        bank_balance_after.amount_minor().abs(), 500_000_00,
...
async fn fail_from_processing_reverses_ledger_entry(pool: PgPool) -> sqlx::Result<()> {
    ...
    assert_ne!(balance_after_process.amount_minor(), 0, "ledger changed at process");

    // Fail: must reverse the ledger entry.
    let failed = fix.payout_engine.fail(payout.id, "bank transfer rejected".into()).await.unwrap();
    ...
    assert_eq!(
        balance_after_fail.amount_minor(), 0,
...
async fn fail_from_pending_writes_no_ledger_entry(pool: PgPool) -> sqlx::Result<()> {
    ...
    let payout = fix.payout_engine.initiate(CreatePayoutRequest {
        ...
        merchant_id:     fix.merchant_id,
        wallet_id:       fix.wallet_id,
        amount:          kz(100_000_00),
        destination:     destination(),
    }).await.unwrap();

    // Fail before processing — no ledger entry should ever be written.
    let failed = fix.payout_engine.fail(payout.id, "cancelled before processing".into()).await.unwrap();
    assert_eq!(failed.status, PayoutStatus::Failed);
    assert!(failed.ledger_posting_id.is_none(), "no posting ID — ledger was never touched");
...
async fn duplicate_initiate_returns_existing_payout(pool: PgPool) -> sqlx::Result<()> {
    ...
    assert_eq!(first.id, second.id, "duplicate initiate must return the same payout");
