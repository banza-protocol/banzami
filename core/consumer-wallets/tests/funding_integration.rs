...
use banzami_consumer_wallets::funding::{
    CreateFundingSessionRequest, FundingEngine, FundingError, FundingProvider, FundingStatus,
    PostgresFundingEngine, ReceiveCallbackRequest, ReconcileRequest,
};
...
) -> (ConsumerWalletId, ConsumerId, AccountId) {
...
fn funding_req(
    ...
    CreateFundingSessionRequest {
        ...
        amount: Money::new(1_000_000, Currency::AOA),
...
async fn happy_path_full_funding_flow(pool: PgPool) {
    ...
    let session = engine
        .create_session(funding_req(consumer_id, wallet_id))
        .await
        .unwrap();
    ...
    let session = engine
        .receive_callback(ReceiveCallbackRequest {
    ...
    .fetch_one(&pool)
    .await.unwrap();
    // LIABILITY CREDIT of 1_000_000 → net = -1_000_000 (consumer's available = +1_000_000)
    assert_eq!(net, -1_000_000_i64);
}
...
async fn inv_wal_004_3_duplicate_callback_rejected(pool: PgPool) {
    ...
    let session2 = engine
        .create_session(CreateFundingSessionRequest {
            consumer_id,
            wallet_id,
            provider: FundingProvider::Simulated,
            amount: Money::new(500_000, Currency::AOA),
    ...
    let count: i64 =
        sqlx::query_scalar("SELECT COUNT(*) FROM provider_callbacks WHERE provider_event_id = $1")
            .bind(&event_id)
            .fetch_one(&pool)
            .await
            .unwrap();
...
async fn delayed_callback_settles_eventually(pool: PgPool) {
    ...
    .fetch_one(&pool)
    .await.unwrap();
    // LIABILITY CREDIT: net = -1_000_000
    assert_eq!(
        net_after, -1_000_000_i64,
        "wallet must be credited after delayed settlement"
    );
}
...
async fn inv_wal_004_4_reversal_preserves_audit_trail(pool: PgPool) {
    ...
    let reversed = engine
        .reverse_session(settled.id, "bank reversal: chargeback".into())
        .await
        .unwrap();
    ...
    let orig_exists: bool =
        sqlx::query_scalar("SELECT EXISTS(SELECT 1 FROM ledger_postings WHERE id = $1)")
    ...
    assert!(
        orig_exists,
        "original posting must be immutable and preserved"
    );

    // Reversal posting must also exist.
    let reversal_posting_id: Option<Uuid> =
        sqlx::query_scalar("SELECT reversal_posting_id FROM consumer_deposits WHERE id = $1")
            .bind(settled.id)
            .fetch_one(&pool)
            .await
            .unwrap();
    assert!(
        reversal_posting_id.is_some(),
        "reversal posting must be linked"
    );
    ...
    assert_eq!(
        net, 0,
        "balance must be zero after reversal offsets original credit"
    );

    // Both postings preserved in ledger — audit trail intact.
    let posting_count: i64 =
        sqlx::query_scalar("SELECT COUNT(*) FROM ledger_postings WHERE id = ANY($1)")
            .bind(vec![
    ...
    assert_eq!(
        posting_count, 2,
        "both original and reversal postings must exist"
    );
}
...
async fn inv_wal_004_5_expired_session_cannot_be_settled(pool: PgPool) {
    ...
    assert!(
        matches!(
            err,
            FundingError::InvalidTransition {
                from: FundingStatus::Expired,
                ..
            }
        ),
...
async fn reconcile_is_idempotent(pool: PgPool) {
    ...
    let first = engine
        .reconcile(ReconcileRequest {
            ...
            transit_account: req.transit_account,
        })
        .await
        .unwrap();
    let second = engine
    ...
    assert_eq!(
        first.ledger_posting_id, second.ledger_posting_id,
    ...
    assert_eq!(
        posting_count, 1,
        "idempotent reconcile must not create duplicate postings"
    );
    ...
    assert_eq!(
        net, -1_000_000_i64,
        "balance credited exactly once despite double reconcile"
    );
}
...
async fn create_session_is_idempotent(pool: PgPool) {
    ...
    let make = || CreateFundingSessionRequest {
        ...
        wallet_id,
        provider: FundingProvider::Simulated,
        amount: Money::new(200_000, Currency::AOA),
        external_ref: format!("REF-{}", Uuid::new_v4()),
    ...
    assert_eq!(
        first.id, second.id,
        "same idempotency_key must return the same session"
    );

    let count: i64 =
        sqlx::query_scalar("SELECT COUNT(*) FROM consumer_deposits WHERE idempotency_key = $1")
            .bind(&idem_key)
            .fetch_one(&pool)
            .await
            .unwrap();
