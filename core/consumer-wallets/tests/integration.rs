...
//
// Tests the full onboarding lifecycle against a real PostgreSQL database:
// PENDING_OTP → PENDING_PIN → ACTIVE.
//
// Invariants verified:
//   INV-WALLET-001  fresh session has PENDING_OTP status
...
use banzami_consumer_wallets::{
    ...
    PostgresOnboardingRepository, StartOnboardingRequest, VerifyOtpRequest,
};
...
fn engine(pool: PgPool) -> impl ConsumerWalletEngine {
    ...
    let wallet_repo  = PostgresConsumerWalletRepository::new(pool);
    PostgresConsumerWalletEngine::new(ledger, onboard_repo, wallet_repo)
}
