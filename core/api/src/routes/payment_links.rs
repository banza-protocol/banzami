...
fn map_err(e: PaymentLinkError) -> ApiError {
    match e {
        ...
        PaymentLinkError::ExpiryInPast =>
            ApiError::bad_request("expires_at must be in the future"),
        PaymentLinkError::Database(_) =>
...
pub async fn create(
    ...
    let merchant_id = body.merchant_id.parse::<MerchantId>()
        .map_err(|_| ApiError::bad_request("invalid merchant_id"))?;
    let wallet_id = body.wallet_id.parse::<WalletId>()
...
pub async fn list(
    ...
    let merchant_id = q.merchant_id.parse::<MerchantId>()
        .map_err(|_| ApiError::bad_request("invalid merchant_id"))?;
...
pub async fn get_by_slug(
    ...
    let link = state.payment_links.get_by_slug(&slug).await.map_err(map_err)?;
    Ok(Json(link.into()))
}
