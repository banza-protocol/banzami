...
// Readiness completes a Business so it can settle; it does not decide who may
// take an application fee. ADR-028's taxonomy is an operator decision, like the
// pricing profile. A version of this route promoted every self-service Business
// from the MERCHANT default to APPLICATION — to get round settlement validating
// a fee destination even when no fee was due — and that was the wrong fix.
#[sqlx::test(migrations = "../../db/migrations")]
async fn readiness_never_classifies_the_business(pool: PgPool) {
    let m = merchant(&pool, "ACTIVE").await;
    let state = state_for(pool.clone(), CoreEnvironment::Sandbox).await;
    let (_, Json(res)) = business_readiness(
        State(state),
    ...
    assert_eq!(
        after.as_deref(),
        Some("MERCHANT"),
        "a developer pressing 'configure' must not grant itself APPLICATION"
    );
    assert_eq!(res.business_account_type, "MERCHANT");
}
