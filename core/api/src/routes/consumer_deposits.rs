...
pub async fn initiate(
    ...
    let ext = state.acquiring
        .initiate_raw(&deposit_id.to_string(), amount.clone())
        .await
        .map_err(|e| match e {
    ...
    Ok((StatusCode::CREATED, Json(ConsumerDepositResponse {
...
pub async fn get(
    ...
    let row: Option<(uuid::Uuid, uuid::Uuid, uuid::Uuid, String, String, String,
                      i64, String, serde_json::Value,
                      Option<DateTime<Utc>>, Option<DateTime<Utc>>, DateTime<Utc>, DateTime<Utc>)> =
        sqlx::query_as(
    ...
    let (id, consumer_id, wallet_id, provider, external_ref, status,
         amount_minor, currency, instructions,
         confirmed_at, failed_at, expires_at, created_at) =
...
pub async fn callback(
    ...
    let confirmation = state.acquiring
    ...
    type DepositRow = (uuid::Uuid, uuid::Uuid, uuid::Uuid, String, String, String,
                       i64, String, serde_json::Value, DateTime<Utc>, DateTime<Utc>);
    ...
    let (dep_id, dep_consumer_id, dep_wallet_id, dep_provider, dep_external_ref, dep_status,
         dep_amount_minor, dep_currency, dep_instructions, dep_expires_at, dep_created_at) =
    ...
    if dep_status != "PENDING" {
        return Ok(Json(ConsumerDepositResponse {
            ...
            instructions: DepositInstructionsResponse {
                method:    dep_instructions["method"].as_str().unwrap_or("").to_string(),
                entity:    dep_instructions["entity"].as_str().unwrap_or("").to_string(),
                reference: dep_instructions["reference"].as_str().unwrap_or("").to_string(),
            },
            confirmed_at: None,
            failed_at:    None,
...
pub async fn test_confirm(
    ...
    let (amount_minor, currency) = deposit
        .ok_or_else(|| ApiError::not_found("no deposit for this external_ref"))?;

    let (raw_body, signature) = state.acquiring
