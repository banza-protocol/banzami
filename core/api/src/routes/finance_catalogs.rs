...
fn map_err(e: PricingError) -> ApiError {
    match e {
        ...
        PricingError::Config(m) if m.contains("already exists") => {
            ApiError::conflict("CONFLICT", m)
        }
...
async fn update(
    ...
    let r = state
        .catalog
        .update(kind, parse_id(id)?, body.into_input())
        .await
        .map_err(map_err)?;
    Ok(Json(serde_json::to_value(&r).unwrap()))
}
...
async fn set_enabled(
    ...
    let r = state
        .catalog
        .set_enabled(kind, parse_id(id)?, enabled)
        .await
        .map_err(map_err)?;
    Ok(Json(serde_json::to_value(&r).unwrap()))
}
...
pub async fn profiles_disable(
    State(s): State<AppState>,
    Path(id): Path<String>,
) -> ApiResult<Json<serde_json::Value>> {
    set_enabled(&s, CatalogKind::PricingProfiles, &id, false).await
}
pub async fn profiles_enable(
    State(s): State<AppState>,
    Path(id): Path<String>,
) -> ApiResult<Json<serde_json::Value>> {
...
pub async fn policies_disable(
    State(s): State<AppState>,
    Path(id): Path<String>,
) -> ApiResult<Json<serde_json::Value>> {
    set_enabled(&s, CatalogKind::FeePolicies, &id, false).await
}
pub async fn policies_enable(
    State(s): State<AppState>,
    Path(id): Path<String>,
) -> ApiResult<Json<serde_json::Value>> {
