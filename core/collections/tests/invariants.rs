...
impl CollectionRepository for MemRepo {
    ...
    async fn find_collection(
        ...
        Ok(self
            ...
            .get(&id.as_uuid())
            .filter(|c| {
                c.merchant_id.as_uuid() == merchant_id.as_uuid() && c.environment == environment
            })
            .cloned())
    }
    ...
    async fn find_share(
        ...
        Ok(self
            ...
            .get(&id.as_uuid())
            .filter(|s| {
                s.merchant_id.as_uuid() == merchant_id.as_uuid() && s.environment == environment
            })
            .cloned())
    }
    ...
    async fn set_share_intent(
        ...
        let s = g
            .get_mut(&id.as_uuid())
            .ok_or(CollectionError::ShareNotFound(id))?;
    ...
    async fn find_payment_intent(
        ...
        Ok(self
            ...
            .get(&id.as_uuid())
            .filter(|p| {
                p.merchant_id.as_uuid() == merchant_id.as_uuid() && p.environment == environment
            })
            .cloned())
    }
    ...
    async fn mark_intent_paid(
        ...
        let p = g
            .get_mut(&id.as_uuid())
            .ok_or(CollectionError::IntentNotFound(id))?;
...
async fn equal_split_exact_creates_balanced_shares() {
    ...
    assert_eq!(
        sum, c.total_amount_minor,
        "INV-COLLECTION-002: sum == total"
    );
    // INV-COLLECTION-001: no money/ledger field on a share.
    assert!(shares
        .iter()
        .all(|s| s.transfer_id.is_none() && s.status == ShareStatus::Pending));
}
...
async fn equal_split_indivisible_rejected_no_rounding() {
    ...
    let err = e
        ...
        .unwrap_err();
    assert!(
        matches!(err, CollectionError::Indivisible),
        "no silent rounding"
    );
}
...
async fn equal_split_remainder_to_first_is_explicit_and_balanced() {
    ...
    assert_eq!(
        sum, c.total_amount_minor,
        "remainder distributed, exact sum"
    );
...
async fn ownership_and_environment_scoping_returns_not_found() {
    ...
    assert!(matches!(
        e.get_collection(c.id, c.merchant_id, "LIVE")
            .await
            .unwrap_err(),
        CollectionError::NotFound(_)
    ));
    // correct scope -> ok
    assert!(e
        .get_collection(c.id, c.merchant_id, "SANDBOX")
        .await
        .is_ok());
}
...
async fn closed_rule_rejects_ad_hoc_shares_open_rule_enforces_minimum() {
    ...
    let (mc, _) = e
        .create_collection(req(
            0,
            CollectionRule::MinimumContribution {
                min_minor: 500_000,
                target_minor: None,
            },
        ))
...
async fn surface_share_creates_intent_and_is_not_repeatable() {
    ...
    assert!(matches!(
        e.surface_share(share_id, c.merchant_id, "SANDBOX", Surface::Qr, None)
            .await
            .unwrap_err(),
...
async fn surface(
    e: &PostgresCollectionEngine<MemRepo>,
...
async fn settlement_first_payment_marks_paid_and_rolls_up_partial() {
    ...
    let out = e
        .settle_from_surface(Surface::Qr, "qr-1", tid, "SANDBOX")
        .await
        .unwrap()
        .unwrap();
    ...
    assert_eq!(
        share.transfer_id,
        Some(tid),
        "INV-COLLECTION-005: real transfer recorded"
    );
    assert_eq!(
        out.collection.unwrap().status,
...
async fn settlement_is_idempotent_no_double_pay_no_double_event() {
    ...
    let other = e
        .settle_from_surface(Surface::Qr, "qr-1", TransferId::new(), "SANDBOX")
        .await
        .unwrap()
        .unwrap();
    assert!(
        !other.newly_paid,
        "INV-COLLECTION-006: PAID is terminal, no double payment"
    );
}
...
async fn settlement_last_share_completes_collection() {
    ...
    let p2 = e
        .settle_from_surface(Surface::Qr, "qr-2", TransferId::new(), "SANDBOX")
        .await
        .unwrap()
        .unwrap();
    assert_eq!(
...
async fn settlement_of_non_collection_surface_is_a_noop() {
    ...
    let out = e
        .settle_from_surface(Surface::Qr, "unrelated-qr", TransferId::new(), "SANDBOX")
        .await
        .unwrap();
